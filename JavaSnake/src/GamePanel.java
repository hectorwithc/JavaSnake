...
public class GamePanel extends JPanel implements ActionListener{
	...
	// Easter eggs
	boolean randomColorsMode = false;
	
	// Settings
	boolean gridMode = false;
	boolean noBordersMode = true;
	...
	public void draw(Graphics g) {
		...
		if(running) {
			...
			// Settings and Easter eggs activated text
			g.setColor(new Color(45, 180, 0));
			g.setFont(new Font("Poppins", Font.BOLD, 15));
			if (noBordersMode) {
				g.drawString("No Borders", (SCREEN_WIDTH/UNIT_SIZE)-1, (SCREEN_HEIGHT - 10));
			}
		}
		else {
			gameOver(g);
	...
	public void checkCollisions() {
		...
		if (noBordersMode) {
			...
			if (x[0] < 0) {
				x[0] = x[0] + (SCREEN_WIDTH + (1 * UNIT_SIZE));
				direction = 'L';
			}
			...
			if (y[0] < 0) {
				y[0] = y[0] + (SCREEN_HEIGHT + (1 * UNIT_SIZE));
				direction = 'U';
			}
